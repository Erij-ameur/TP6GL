...
class TicketMachineTest {
	...
	public void imprimeEtMaJTRUE(){
		machine.insertMoney(PRICE);
		machine.printTicket();
		assertEquals(machine.getTotal(),50);
	}

	@Test
	//S6 Le montant collecté est mis à jour quand on imprime un ticket(pas avant)
	//!!!!!! CAS INCORRECT
	public void imprimeEtMaJFALSE(){
		machine.insertMoney(PRICE);
		assertEquals(machine.getTotal(),50);
	}
	...
	@Test
	//S9 on peut pas inserer un montant negatif CAS TRUE
	public void NegativeAmountTRUE(){
	...
	public void NegativeTicketTRUE(){
	...
	//S10 on ne peut pas créer de machine qui délivre des tickets dont le prix est négatif 
	//!!!!!! CAS INCORRECT
	public void NegativeTicketFALSE(){
		machine = new TicketMachine(-20);
	}
